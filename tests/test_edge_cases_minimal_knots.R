...
# Test 1: Check if 2 knots gives appropriate error
cat("Test 1: Checking error handling for insufficient knots (2 knots)\n")
cat("-----------------------------------------------------------------\n")

# B-splines with 2 knots (should fail)
stan_data_2knots <- list(
  n_data = n,
  x = x,
  y = y,
  num_knots = 2,  # This should be minimum but let's see
  spline_degree = 3,
...
cat("Testing B-splines with 2 knots... ")
tryCatch({
  ...
  cat("UNEXPECTED: B-splines with 2 knots succeeded!\n")
}, error = function(e) {
  cat("Expected behavior: B-splines with 2 knots failed\n")
  cat("Note: 2 knots is the minimum (boundaries only), but may not provide enough flexibility\n")
})

# C-splines with 2 knots (should fail)
stan_data_c_2knots <- list(
  n_data = n,
  x = x,
  y = y,
  num_knots = 2  # Should fail - need at least 3 for natural cubic splines
)

model_c <- cmdstan_model("code/csplines.stan")

cat("\nTesting C-splines with 2 knots... ")
tryCatch({
  ...
  if (grepl("num_knots must be at least 2", e$message)) {
    cat("Note: C-splines allows 2 knots but may not be practical\n")
  } else {
    cat("Expected: C-splines with 2 knots failed\n")
    cat("Error: ", e$message, "\n")
  }
})

# Test 2: B-splines with 3 knots
cat("\n\nTest 2: B-splines with 3 knots (minimal practical configuration)\n")
cat("-----------------------------------------------------------------\n")
...
p_b <- ggplot() +
  ...
  geom_line(data = data.frame(x = x_true_hires, y = y_true_hires), 
            aes(x = x, y = y_true), color = "red", linetype = "dashed", linewidth = 1) +
  labs(
...
combined_plot <- p_b / p_c +
  plot_annotation(
    ...
    subtitle = "Both spline types struggling with complex function due to limited flexibility",
    caption = "Red dashed = true function, Black points = data, Colored lines = fitted splines with 95% CI"
  )

# Save plot
dir.create("output", showWarnings = FALSE)
ggsave("output/test-edge_cases_minimal_knots.png", combined_plot, width = 8, height = 10, dpi = 300)
cat("\nSaved comparison plot to output/test-edge_cases_minimal_knots.png\n")

# Summary
cat("\n\nSummary of Edge Case Testing\n")
cat("============================\n")
cat("1. Minimum knots: Both B-splines and C-splines require at least 2 knots\n")
cat("2. Practical minimum: 3 knots provides minimal flexibility\n")
cat("3. B-splines with 3 knots: Creates 5 basis functions (knots + degree - 1)\n")
cat("4. C-splines with 3 knots: Very limited flexibility for complex functions\n")
cat("5. Both struggle with the complex test function due to insufficient flexibility\n")
cat("\nRecommendation: Use adaptive knot selection (n/2 rule) for better fits\n")
