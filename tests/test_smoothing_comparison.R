...
cat("---------------------------------------------------------------\n")
smoothing_values <- c(0, 2, 4, 6, 8)  # New scale: 0=none, 2=mild, 4-6=moderate, 8=strong
...
combined_plot <- (wrap_plots(plots_b, ncol = 1) | wrap_plots(plots_c, ncol = 1)) +
  plot_annotation(
    ...
    subtitle = paste0("B-splines: Fixed ", fixed_knots_b, " knots (n/2), varying smoothing_strength | C-splines: Varying number of knots"),
    caption = "Red dashed line = true function | Blue/Green = fitted spline with 95% CI"
  )

# Save plot
dir.create("output", showWarnings = FALSE)
ggsave("output/test-smoothing_comparison.png", combined_plot, width = 10, height = 14, dpi = 300)
cat("Saved comparison plot to output/test-smoothing_comparison.png\n")
...
ggsave("output/test-smoothing_comparison_summary.png", summary_plot, width = 10, height = 8, dpi = 300)
cat("Saved summary plot to output/test-smoothing_comparison_summary.png\n")

# Print summary table
cat("\n5. Summary Table\n")
cat("================\n\n")
cat(paste0("B-splines (", fixed_knots_b, " knots, varying smoothing_strength):\n"))
